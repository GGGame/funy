...
var GameLayer = cc.Layer.extend({
    ...
    init:function () {
        ...
        if (this._super()) {
            ...
            cc.log('-------' + (typeof cc.SizeMake))
            // score
           this.lbScore = cc.LabelTTF.create("Score: 0", "Arial",14,{height:80,width:14},cc.TEXT_ALIGNMENT_RIGHT);
    ...
    onTouchesBegan:function(touches, event){
        cc.log('touch begin');
        _glGlobal._isTouch = true;
    },
    onTouchesMoved:function (touches, event) {
        cc.log('touch move');
        if(_glGlobal._isTouch){
            _glGlobal.processEvent(touches[0]);
        }
    },
    onTouchesEnded:function(touches, event){
        cc.log('touch end');
        _glGlobal._isTouch = false;
    },
    onMouseDragged:function( event ) {
        if(_glGlobal._isTouch){
            _glGlobal.processEvent( event );
        }
    },

    processEvent:function( event ) {
        cc.log('touch processEvent');
        if( _glGlobal._state == STATE_PLAYING ) {
            var delta = event.getDelta();
            var curPos = _glGlobal._ship.getPosition();
            curPos= cc.pAdd( curPos, delta );
            curPos = cc.pClamp(curPos, cc.POINT_ZERO, cc.p(winSize.width, winSize.height) );
            _glGlobal._ship.setPosition( curPos );
        }
    },
